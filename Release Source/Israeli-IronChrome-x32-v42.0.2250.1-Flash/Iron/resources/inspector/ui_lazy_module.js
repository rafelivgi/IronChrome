...
this._headerRow.appendChild(this._headerTableHeaders[columnIdentifier]);this._topFillerRow.createChild("td","top-filler-td");this._bottomFillerRow.createChild("td","bottom-filler-td");}
...
this.scheduleUpdate();},scheduleUpdate:function()
...
{this._inline=true;WebInspector.DataGrid.prototype.renderInline.call(this);this._update();},_calculateVisibleNodes:function(clientHeight,scrollTop)
{var nodes=this._rootNode.children;if(this._inline)
...
{var nodes=this._rootNode.children;var result=0;for(var i=0,size=nodes.length;i<size;++i)
...
scrollTop=maxScrollTop;scrollTop=Math.min(maxScrollTop,scrollTop);this._atBottom=scrollTop===maxScrollTop;var viewportState=this._calculateVisibleNodes(clientHeight,scrollTop);var visibleNodes=viewportState.visibleNodes;var visibleNodesSet=Set.fromArray(visibleNodes);if(this._hiddenWheelTarget&&this._hiddenWheelTarget!==this._wheelTarget){this._hiddenWheelTarget.remove();this._hiddenWheelTarget=null;}
...
{var nodes=this._rootNode.children;var index=nodes.indexOf(node);if(index===-1)
...
return(this._element);},insertChild:function(child,index)
{child.parent=this;child.dataGrid=this.dataGrid;this.children.splice(index,0,child);child.recalculateSiblings(index);this.dataGrid.scheduleUpdate();},removeChild:function(child)
{child.deselect();this.children.remove(child,true);if(child.previousSibling)
child.previousSibling.nextSibling=child.nextSibling;if(child.nextSibling)
child.nextSibling.previousSibling=child.previousSibling;this.dataGrid.scheduleUpdate();},removeChildren:function()
{for(var i=0;i<this.children.length;++i)
this.children[i].deselect();this.children=[];this.dataGrid.scheduleUpdate();},expand:function()
{},willAttach:function(){},attached:function()
...
children[i].recalculateSiblings(i);this.scheduleUpdate();},__proto__:WebInspector.ViewportDataGrid.prototype}
...
{var hash=id.hashCode();var h=this._indexToValueInSpace(hash,this._hueSpace);var s=this._indexToValueInSpace(hash,this._satSpace);var l=this._indexToValueInSpace(hash,this._lightnessSpace);var a=this._indexToValueInSpace(hash,this._alphaSpace);return"hsla("+h+", "+s+"%, "+l+"%, "+a+")";},_indexToValueInSpace:function(index,space)
...
return;var inDividersBar=event.offsetY<WebInspector.FlameChart.DividersBarHeight;this._highlightedMarkerIndex=inDividersBar?this._markerIndexAtPosition(event.offsetX):-1;this._updateMarkerHighlight();if(inDividersBar)
